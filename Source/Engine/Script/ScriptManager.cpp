...
int ScriptManager::compile(ScriptComponent& script)
{
    ...
    r = builder.AddSectionFromFile(std::string(script.script).c_str());
    if (r < 0)
    {
        spdlog::warn("ScriptManager - Failed to add script file " + std::string(script.script));
        return r;
    }
    ...
    if (r >= 0)
    {
        ...
        func = m_engine->GetModule(script.module.c_str())->GetFunctionByDecl(_entity_script_on_player_collide);
        if (func)
        {
            script.hasOnPlayerCollide = true;
            script.onPlayerCollideFunc = func;
        }

        func = m_engine->GetModule(script.module.c_str())->GetFunctionByDecl(_entity_script_npc_update);
        if (func)
        {
            script.hasNpcUpdate = true;
            script.npcUpdateFunc = func;
        }
