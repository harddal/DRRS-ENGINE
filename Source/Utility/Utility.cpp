...
std::string Utility::SaveFileDialog(const char* filter, const char* initialDir)
{
	char filename[MAX_PATH];
	...
	// If the filename has no extension, append the one from the selected filter entry.
	// The filter string is pairs of null-terminated strings: "Description\0*.ext\0..."
	// nFilterIndex is 1-based, so we skip (nFilterIndex - 1) pairs to reach the right one.
	char ext[_MAX_EXT];
	_splitpath(result.c_str(), nullptr, nullptr, nullptr, ext);
	if (ext[0] == '\0' && filter != nullptr)
	{
		const char* p = filter;
		DWORD pairIndex = 0;
		while (*p)
		{
			...
			// p now points at the pattern string, e.g. "*.png"
			if (++pairIndex == ofn.nFilterIndex)
			{
				// Extract extension from pattern like "*.png" -> ".png" -> "png"
				const char* dot = strchr(p, '.');
				if (dot && dot[1] != '\0' && dot[1] != '*')
					result += dot; // appends e.g. ".png"
				break;
			}
			p += strlen(p) + 1;
		}
