...
#define dialog_filter_scene_zip  "Scene Package\0*.pak\0Scene Files\0*.scn\0Any File\0*.*\0"
#define dialog_filter_entity "Entity Files\0*.ent\0Any File\0*.*\0"
#define dialog_filter_script "Script Files\0*.asc\0Any File\0*.*\0"
#define dialog_filter_mesh   "Object\0*.obj\0Gltf Binary\0*.glb\0FBX\0*.fbx\0DirectX Mesh\0*.x\0B3D Mesh\0*.b3d\0Any File\0*.*\0"
...
namespace Utility
{
	...
	inline std::string RemoveAbsDir(const std::string &path) { if (path.empty()) return std::string(); else return "content" + GetRightSideOfDelim(path, "content"); }
...
namespace Math
{
	...
	inline irr::core::vector3df QuaternionToEuler(const physx::PxQuat& quaternion)
	{
		...
		double sinY = -2.0 * (x * z - y * w);
		sinY = sinY < -1.0 ? -1.0 : (sinY > 1.0 ? 1.0 : sinY);
		euler.Y = static_cast<irr::f32>(asin(sinY) * (180.0f / __pi));
