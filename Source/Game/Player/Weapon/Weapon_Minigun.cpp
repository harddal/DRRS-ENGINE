...
void Weapon_Minigun::init()
{
	...
	auto* shellMesh = RenderManager::Get()->sceneManager()->getMesh("content/mesh/prop/shells/shelllarge.obj");
	...
	for (int i = 0; i < SHELL_POOL_SIZE; i++)
	{
		...
		if (m_shellPool[i].node)
		{
			...
			m_shellPool[i].node->setRotation(irr::core::vector3df(0.0f, 0.0f, 0.0f));
...
void Weapon_Minigun::createMuzzleFlash()
{
	if (!m_mesh.node)
		return;
...
void Weapon_Minigun::updateMuzzleFlash(float dt)
{
	if (!m_muzzleStarNode)
		return;
...
void Weapon_Minigun::createTracerBeam(const irr::core::vector3df& start, const irr::core::vector3df& end)
{
...
void Weapon_Minigun::ejectShell()
{
	if (!m_mesh.node)
		return;
	...
	irr::scene::IBoneSceneNode* ejectBone = m_mesh.node->getJointNode("EJECT");
	if (!ejectBone)
	{
		spdlog::warn("Eject bone not found on minigun model - cannot EJECT shell");
		return;
	}
...
void Weapon_Minigun::updateTracers(float dt)
{
	float currentTime = Engine::Get()->getCurrentTime();
...
void Weapon_Minigun::updateShells(float dt)
{
...
// ---------------------------------------------------------------------------
void Weapon_Minigun::initImpactSparkSystem()
{
...
void Weapon_Minigun::createImpactEffect(const irr::core::vector3df& pos)
{
	if (m_impactSparkBaseID == SPK::NO_ID)
		return;
