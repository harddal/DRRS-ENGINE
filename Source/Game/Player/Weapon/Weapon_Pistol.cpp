...
void Weapon_Pistol::destroy()
{
	...
	for (auto* sys : m_impactSystems)
		SPK_Destroy(sys); // onUnregister() auto-removes from LDR effect list
...
void Weapon_Pistol::persist()
{
	...
	for (auto it = m_impactSystems.begin(); it != m_impactSystems.end();)
	{
		...
		{
			SPK_Destroy(*it); // onUnregister() auto-removes from LDR effect list
...
// ---------------------------------------------------------------------------
void Weapon_Pistol::initImpactSparkSystem()
{
...
void Weapon_Pistol::createImpactEffect(const irr::core::vector3df& pos)
{
	if (m_impactSparkBaseID == SPK::NO_ID)
		return;
	...
	m_impactSystems.push_back(system); // onRegister() already added it to LDR effect list
}